 ...
 *  strswitch.h
 *
 *  Created by Efrit on 23/12/13.
 *  More about you can find at page https://habr.com/post/166201/.
 *
 */
...
#define CASE(str)                                                              \
  static_assert(s_s::str_is_correct(str) &&                                    \
                    (s_s::str_len(str) <= s_s::MAX_LEN),                       \
                "CASE string contains wrong characters, or its length is "     \
                "greater than 9");                                             \
...
constexpr bool str_is_correct(const char *const str) {
  return (static_cast<signed char>(*str) > 0) ? str_is_correct(str + 1)
                                              : (*str ? false : true);
}
...
inline ullong str_hash_for_switch(const char *const str) {
  return (str_is_correct(str) && (str_len(str) <= MAX_LEN))
             ? str_hash(str, str_len(str))
             : N_HASH;
}

inline ullong str_hash_for_switch(const std::string &str) {
  return (str_is_correct(str.c_str()) && (str.length() <= MAX_LEN))
             ? str_hash(str.c_str(), str.length())
             : N_HASH;
}
} // namespace s_s
